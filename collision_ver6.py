...
class Particle:
	...
	def Overlap(self, target):
		return np.hypot(*(self.pos - target.pos)) <= (self.rad + target.rad)*1.2
...
class Simulation:
	...
	def collision(self, p1, p2):
		...
		u1 = p1.vel - 2*p2.mass / M * np.dot(p1.vel - p2.vel, p1.pos - p2.pos) / d * (p1.pos - p2.pos)
		u2 = p2.vel - 2*p1.mass / M * np.dot(p2.vel - p1.vel, p2.pos - p1.pos) / d * (p2.pos - p1.pos)
	...
	def InitFile(self):
		self.f = open("./test.txt", "ab")

	def WriteFile(self):
		data = np.sqrt(np.array([p.vx**2 for p in self.particles]) + np.array([p.vy**2 for p in self.particles]))
		np.savetxt(self.f, data)

if __name__=="__main__":
	wow = Simulation(50, 50, 0.5)
